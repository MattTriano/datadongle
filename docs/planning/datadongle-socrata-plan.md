...
- Other ~11 collectors still on the legacy `staged_ingest` / `collect(spec, force)` path; migrate each onto a `SourceReader` + the shared driver (Socrata + OSM are the templates — OSM covers the SCD2/`invalidate_missing` + non-source-column HWM case).
